...
BEGIN { 
    ...
    if (eval { local $SIG{__DIE__}; require Devel::StackTrace::WithLexicals; 1 }) {
...
sub call {
    ...
    # this won't be installed by the .psgi, in the default config, unless $env->{'webgui.debug'} is true
    ...
    # if( $@ and blessed $@ and my $trace = $@->trace ) 
# warn 'XXX: $@ and blessed $@ and my $trace = $@->trace are all true';
