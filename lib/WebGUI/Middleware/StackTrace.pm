...
warn "XXX our stacktrace in";
    ...
    if ($trace) {
        ...
        my $html = eval { trace_as_html($trace, $env->{'webgui.session'}) } || $@ || 'no output at all ever even a little from trace_as_html XXX';
...
warn "running local stacktrace call XXX";
...
use Carp;
warn "XXX getting ready to try: ". Carp::longmess;

        local $SIG{__DIE__} = sub {
            warn "local stacktrace die ran XXX";
            $trace = $StackTraceClass->new;
            die @_;
        };
        ...
        $caught = $_ ; 
        warn "local stacktrace catch ran XXX"; 

warn "local stacktrace apparently cleaning stuff up on the way back down the call stack XXX";
warn "local stacktrace:  trace:  $trace  caught: $caught";

        # if ($trace && ($caught || ($self->force && ref $res eq 'ARRAY' && $res->[0] == 500)) )  # XXX
        if ($trace) {
            my $text = eval { trace_as_string($trace) } || $@;
            my $html = eval { trace_as_html($trace, $env->{'webgui.session'}) } || $@ || 'no output at all ever even a little from trace_as_html XXX';
            $env->{'psgi.errors'}->print($text) unless $self->no_print_errors;
            if (($env->{HTTP_ACCEPT} || '*/*') =~ /html/) {
warn "local stacktrace creating an html response XXX";
                $res = [500, ['Content-Type' => 'text/html; charset=utf-8'], [ utf8_safe($html) ]];
            } else {
                $res = [500, ['Content-Type' => 'text/plain; charset=utf-8'], [ utf8_safe($text) ]];
            }
...
use Carp;
warn "XXX getting ready to try: ". Carp::cluck;

    $SIG{__DIE__} = sub {

        warn "local stacktrace die running XXX";
        $caught = join '', @_; # $_[0]; 
        $trace = $StackTraceClass->new;
...
warn "local stacktrace leaving XXX";
...
#    return $res;
    ...
    #    my $caught = $_;
        ...
        warn "local stacktrace response_cb ran XXX"; 
...
warn "running local trace_as_html XXX";
